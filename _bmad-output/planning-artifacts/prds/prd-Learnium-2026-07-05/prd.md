...
- **Streak** — Count of consecutive days on which the user met their Daily Goal. Resets on a missed day (subject to the streak-protection mechanic, §9 Q2).
...
Meeting the Daily Goal increments the Streak once per day; a day with the goal unmet ends the Streak (subject to §9 Q2 streak-protection decision). Realizes UJ-3.
...
#### FR-9: Daily reminder
A user can enable a daily reminder at a chosen time; the reminder deep-links to the fastest goal-meeting action (usually a Review Session). [ASSUMPTION: v1 channel is email, since v1 is web-only; push arrives with any future mobile app.]
...
Free-tier users have fixed monthly Set-generation and Study-Buddy-chat Quotas; Plus lifts them. Quota state (remaining / reset date) is always visible to the user. [ASSUMPTION — pricing placeholder: Free = 5 Set generations + 30 Buddy messages/month; Plus = unlimited generations (fair-use capped) + high chat cap, ~$9.99/mo — confirm against unit economics before launch.]
...
**Privacy.** Chat logs and learning history are personal data: private by default (FR-19), deletable (FR-23), never used to train third-party models, and never exposed via Leagues beyond display name + XP (FR-14). GDPR-grade data-subject rights (export, deletion) apply from launch. Minimum age 16 — students aren't the target and this avoids COPPA/parental-consent scope. [ASSUMPTION — confirm age gate.]

**Cost.** Every LLM-touching action is Quota-gated, cache-checked (semantic cache before model calls), or generation-deferred (Paths generate Sets lazily, FR-17). LLM cost per weekly-active user is a tracked counter-metric (SM-C3). Free-tier Quotas are the cost throttle; they may be tuned post-launch without a PRD change.

## 7. MVP Scope

### 7.1 In Scope (launch)

All of §4, phased as follows. [ASSUMPTION — proposed phasing, confirm:]

- **Phase A — Harden & retain (pre-launch):** §4.1–4.3 production-hardening, §4.4 Streaks/Daily Goals, §4.5 XP/Levels/Badges, §4.7 Review Sessions, §4.10 tier enforcement. Rationale: the retention loop must exist *at* launch, not after — retention collapse is a launch-week failure mode, and Review is the cheapest daily action.
...
- Native mobile apps — web-responsive carries v1; revisit after retention data. `[NOTE FOR PM]` Streak mechanics are materially stronger with push notifications; if D7 retention underperforms, mobile (or PWA + web push) moves up.
- User-authored or edited course content — UGC moderation burden not justified pre-PMF.
- Human content review pipeline — v1 relies on automated checks + report loop (§6).
- Certificates/credentials, enterprise features, offline mode, localization beyond generating content in the user's input language.

## 8. Success Metrics

[ASSUMPTION: all targets are founder-calibration placeholders — confirm against baseline data after 4 weeks live.]

**Primary**
- **SM-1: Activation** — % of new signups who complete ≥1 Lesson in their first session. Target ≥ 60%. Validates FR-1, FR-4 (UJ-1 is the funnel).
- **SM-2: D7 retention** — % of activated users returning and earning XP on day 7. Target ≥ 25%. Validates FR-7–FR-9, FR-15–FR-16.
- **SM-3: Free→Plus conversion** — % of users hitting a Quota wall who subscribe within 7 days. Target ≥ 5% of wall-hitters (≈2–3% of actives). Validates FR-21, FR-22 (UJ-4).

**Secondary**
- **SM-4: Streak adoption** — % of WAU with a Streak ≥ 3 days. Target ≥ 35%. Validates FR-7, FR-8.
- **SM-5: Review habit** — % of daily goal-meeting sessions that include a Review Session. Target ≥ 40%. Validates FR-15, FR-16.
- **SM-6: Set completion rate** — % of generated Sets that reach completion within 30 days. Target ≥ 30%. Validates FR-3, FR-4, FR-5 (a proxy for content quality: bad content doesn't get finished).

**Counter-metrics (do not optimize)**
- **SM-C1: Streak anxiety churn** — % of users who quit within 3 days of losing a Streak ≥ 7 days. If > 20%, the streak-loss mechanic (Q2) needs softening. Counterbalances SM-2/SM-4.
- **SM-C2: Content report rate** — reports per 1,000 Lesson views. Alert threshold > 5. Counterbalances SM-1 (fast generation must not mean sloppy generation).
- **SM-C3: LLM cost per WAU** — must not exceed a fixed ceiling set by unit economics (addendum). Counterbalances SM-1/SM-3 growth pushes.
- **SM-C4: XP integrity** — anomalous XP accrual (e.g., > 3σ daily XP) per League cohort. League competition must not be winnable by farming. Counterbalances SM-4.

## 9. Open Questions

1. **Pricing point and exact Quota numbers** for Free and Plus (FR-21 placeholder) — needs a unit-economics pass on LLM cost per Set/chat.
2. **Streak protection mechanic** — streak freeze (earned/purchased), one-tap repair, or hard loss? Directly shapes SM-C1. (FR-8)
3. **League cohorting details** — number of tiers, promotion/demotion counts, timezone handling for the weekly reset. (FR-13)
4. **Lesson interior format** — text + embedded check questions only, or richer blocks (images, diagrams)? Affects generation cost and quality floor. (FR-3)
5. **Reminder channel** — is email effective enough at v1, or is web push (PWA) worth pulling into Phase A? (FR-9)
6. **Product name** — is "Learnium" the launch name?

## 10. Assumptions Index

*Every inline `[ASSUMPTION]`, surfaced for confirmation:*

1. §2.1 — Social JTBD is secondary at launch (not the acquisition hook).
2. §2.3 — All four UJs were authored, not user-narrated; personas and beats need confirmation.
3. §4.1 FR-1 — Sets contain 4–12 Lessons; generation SLA 60 seconds.
4. §4.4 FR-7 — Day boundaries at user-local midnight.
5. §4.4 FR-9 — v1 reminder channel is email (web-only launch).
6. §4.6 — Duolingo-style League tiers, cohort ~30, weekly cycle; standings refresh on page load is enough.
7. §4.7 FR-15 — Spaced-repetition algorithm choice deferred to architecture.
8. §4.8 FR-17 — Paths span 3–8 Sets; quota accounting = 1 unit for outline, 1 per subsequent Set.
9. §4.9 — Social launch slice is thin (profiles + share cards + friends leaderboard); phased to Phase C.
10. §4.9 FR-20 — Mutual-consent friendship model.
11. §4.10 FR-21 — Placeholder pricing: Free = 5 generations + 30 chats/mo; Plus ≈ $9.99/mo.
12. §5 — No native mobile apps in v1 (responsive web only).
13. §6 — No human content review at launch; report loop + automated checks suffice. Age gate 16+.
14. §7.1 — Three-phase launch sequencing (A: retention core, B: Leagues+Paths, C: social).
15. §8 — All metric targets are placeholders pending baseline data.
